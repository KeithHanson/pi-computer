...
echo "submitting unauthenticated browser task"
...
set +e
curl -sS -N --max-time 2 -D /tmp/pi-computer-api-sse.headers "${API_BASE}/v1/tasks/${task_id}/events" -o /tmp/pi-computer-api-sse.body
sse_rc=$?
set -e
...
cancel_json="$(curl -fsS -X POST "${API_BASE}/v1/tasks/${task_id}/cancel")"
